...
## AMENDMENT A2 — 2026-08-02, pre-final-outcome, non-regression scope clarification

The frozen hash table of §7 covered 1015 files including working-draft figure PDFs and
executable review-contract documents. The Level-3 contract simultaneously requires refining
working figures and restructuring the manuscript, and the release suite's semantic contract
asserts the *location* of sentences that the mandated restructure legitimately moves to the
Supplementary Information. Scope is therefore clarified BEFORE any final outcome exists:
the STRICT byte-freeze zone is scientific evidence — `codes/results/**`, `codes/data/*`, and
every `manuscript/source_data/` payload directory. RECORDED-MUTABLE (changes allowed, every
before/after hash reported by `--verify`): working-draft figure PDFs/PNGs under
`manuscript/figures/`, figure artist manifests, the publication semantic contract, the
figure-evidence audit, the source-data README/licence files and the journal checklist. No
result, checkpoint, source-data payload, or frozen companion evidence file moves out of the
strict zone. The amended freeze-script hash is recorded in `PREREG_HASHES.txt`.

---

## v2 AMENDMENT (incl. A3) — 2026-08-02, frozen AFTER rehearsal, BEFORE any FINAL contact

The rehearsal (development window [560,740], 19 frames) completed at 0.777 GPU-h. The four
§8 parameters are frozen:

1. `matched_noise`: r_m = (1 + 0.09202)/2 = **0.54601**, ell_c = **2.268** cells (v1 formulas,
   rehearsal-measured closure fidelity and error correlation length).
2. m₅ = **0.02** (v1 formula: max(0.02, 2 × 0.000776)).
3. C9 band = **buffer** (frozen v1 rule: the exact arm improves flux tracking more robustly
   there; rehearsal t = 3.02 buffer vs 2.75 log; both bands reported at final).
4. t\* = **4.5** (simulation-calibrated; `block_inference_calibration.json`).

**A3 — C5 reference clarification (development evidence only).** The rehearsal measured a
systematic error-structure premium above the white-noise ladder interpolant for EVERY
correlated-error arm: closure +0.0124, the ℓ=6 correlated probe +0.013, the learned data-only
regressor +0.019. A white-noise curve alone therefore under-predicts any estimator whose
errors are spanwise-correlated — which is a property of supplied-information content, not of
generator-side decoding (the generator never saw any of these arms in training). C5 is
accordingly adjudicated against the max of (white-curve prediction, structure-matched
`matched_noise` gain) + m₅, with the raw white-curve residual reported alongside. This is
frozen now, before the FINAL window is contacted; the decision applicator implementing it is
hash-recorded below. Direction-of-bias note: the reference can only move UP relative to v1 for
correlated-error arms, making a spurious `SIDE_CHANNEL_DETECTED` conviction less likely while
leaving genuine above-reference decoding fully detectable.

**Design-freeze evidence recorded (development window, not claims):** dose curve monotone
(Spearman 1.0), exact ceiling +0.191 (t=18.5), closure +0.138 (t=8.9, absolute skill positive
both seeds), equilibrium +0.063, wrong-time −0.086 (t=−9.1), shuffle −0.009; engineering flux
tracking improves 34% under exact in the buffer band at t=3.0 with 19 frames — the C9 clause
at the final 46-frame design is expected near the calibrated threshold and its risk is
accepted rather than re-engineered.

FINAL launch command (frozen): `--phase final --matched-r 0.54601 --matched-ell 2.268`.

### A2b — release-suite regenerated outputs (same rationale as A2)

`codes/reproduce_all.sh` regenerates its verification products
(`submission_verification.json`, `revision_semantic_verification.json`,
`level3_reader_consequence_verification.json`, `figure_legibility.json` and the node_009
recompute/calibration outputs) on every run; the operator mandate requires re-running the
complete suite, so byte-freezing these outputs is self-contradictory. They are
recorded-mutable with before/after hashes reported. No producer result, checkpoint,
source-data payload or companion evidence file is affected.
